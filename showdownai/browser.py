...
from exceptions import UserNotOnlineException, GameOverException, TierException

class Selenium():
    BASE_URL="http://play.pokemonshowdown.com"
    #BASE_URL = "https://play.pokemonshowdown.com/battle-gen6ou-781630789"
    #BASE_URL="http://frost.psim.us"
    def __init__(self, driver_path, url=BASE_URL, timer_on=False, proxy=False, browser='chrome'):
        self.url = url
        ...
        self.driver = webdriver.Chrome(executable_path=self.driver_path+'/chromedriver.exe', chrome_options = options)
        if browser == "firefox":
            ...
            if proxy:
                ...
                if browser == "chrome":
                    ...
                    self.driver = webdriver.Chrome(executable_path=self.driver_path+'/chromedriver', chrome_options=chrome_options)
                ...
                self.driver = webdriver.PhantomJS(executable_path = self.driver_path+'/phantomjs')
                self.driver.set_window_size(1400,1000)
    ...
    def select_team(self):
        team_select = self.driver.find_element_by_css_selector(".select.teamselect")
        team_select.click()
        self.screenshot('log.png')
        more_team = self.driver.find_element_by_css_selector(".button[name='moreTeams']")
        more_team.click()
        self.screenshot('log.png')
        index = 0
        while not self.check_exists_by_css_selector("button[name='selectTeam'][value='%s']" % str(index)):
            index += 1
            if index > 50:
                print("Cant find team")
                exit()
        choose_team = self.driver.find_element_by_css_selector("button[name='selectTeam'][value='%s']" % str(index))
        choose_team.click()
        self.screenshot('log.png')
        time.sleep(1)

    def choose_tier(self, tier='gen6ou'):
        try:
    ...
    def start_ladder_battle(self):
        ...
        battle.click()
        battle_click = True
        self.driver.save_screenshot('ladder_battle.png')
        time.sleep(1)
...
#         lobby_quit.click()
    ...
    def make_team(self, team):
        ...
        import_button.click()
        self.screenshot('log.png')
        textfield = self.driver.find_element_by_css_selector("textarea.textbox")
        textfield.send_keys(team)
        self.screenshot('log.png')
        save = self.driver.find_element_by_css_selector(".savebutton[name='saveImport']")
        save.click()
        self.screenshot('log.png')
        close_button = self.driver.find_element_by_css_selector(".closebutton[value='teambuilder']")
        close_button.click()
        self.screenshot('log.png')
        time.sleep(2)
    ...
    def get_my_primary(self):
        imgs = self.driver.find_elements_by_css_selector(".innerbattle img")
        for x in imgs:
            if x.is_displayed() == False:
                imgs.remove(x)
        img = imgs[1]
    ...
    def switch(self, index, backup_switch, use_backup=True):
        ...
        if self.check_alive():
            i = self.poke_map[index]
            buttons = self.driver.find_elements_by_css_selector("[name='chooseSwitch']")
            try:
                buttons[i].click()
            except:
    ...
    def get_log(self):
        #print(self.driver)
        try:
            log = self.driver.find_elements_by_class_name("battle-history")
        except:
            #self.driver.refresh()
            #time.sleep(2)
    ...
    def wait_for_move(self):
        ...
        while move_exists == False:
            ...
            time.sleep(2)
            move_exists = self.check_exists_by_css_selector(".movemenu") or self.check_exists_by_css_selector(".switchmenu")
            if self.check_exists_by_css_selector(".button[name='saveReplay']") or self.check_exists_by_css_selector(".button[name='instantReplay']"):
                self.chat("gg")
    ...
    def test(self):
        #self.driver.current_url = "https://play.pokemonshowdown.com/battle-gen7ou-780721968"
        self.start_driver()
        time.sleep(1)
        img = self.driver.find_elements_by_css_selector(".innerbattle img")
        for i in range(len(img)):
            text = img[i].get_attribute('src')
            poke = text.split("/")[-1]
            poke = poke[:-4]
            print(poke)
