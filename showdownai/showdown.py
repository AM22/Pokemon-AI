...
#from gamestateEdited import GameState, Team, Pokemon
from agent import OptimisticMinimaxAgent, BaselineAgent, PessimisticMinimaxAgent
...
import logging
import os
DIR1 = './hubbbb/MinimaxAgent' #è¦�ç»Ÿè®¡çš„æ–‡ä»¶å¤¹
DIR2 = './epsilonnn/BaselineAgent'
...
class Showdown():
    def __init__(self, team_text, agent, username, pokedata, password=None,
                 monitor_url=None, proxy=False, browser='chrome', predictor_name='PokeFrequencyPredictor',
        ...
        self.selenium = Selenium(proxy=proxy, browser=browser, driver_path = r"C:\Users\AM22\AppData\Local\Programs\Python\Python36")
    ...
    def correct_gamestate(self, gamestate):
        ...
        if my_team.primary().health == 0:
            my_team.primary().alive = False;
        opp_team.primary().health = opp_poke_health / 100.0 * opp_team.primary().final_stats['hp']
        if opp_team.primary().health == 0:
            opp_team.primary().alive = False;
        return gamestate
    ...
    def run(self, num_games=1, challenge=None):
        ...
        for i in range(num_games):
            ...
            try:
                ...
                self.logger.info(over_event.details)
            ...
            self.selenium.screenshot("over"+id+".png")
            battle_url = "http://replay.pokemonshowdown.com/%s" % id
            self.logger.info("Finished game! Replay can be found at: %s", battle_url)
            user_folder = Path(".") / self.username / self.agent.str()
            ...
            if result == True:
                ...
                with open(user_folder / "wins" / ("%s.score" % id), 'w') as fp:
                    ...
                    print(self.simulator.total, file = fp)
                '''
                self.reset()
            elif result == False:
                print("---------------")
...
def main():
    ...
    argparser.add_argument('--browser', type=str, default='chrome')
    ...
    argparser.add_argument('--agent', default='MinimaxAgent')
    ...
    with open("../teams/" + args.team + ".txt") as fp:
        team_text = fp.read()

    pokedata = load_data(args.data_dir)

    if args.agent == 'BaselineAgent':
        ag = BaselineAgent(pokedata)
    else:
        ag = PessimisticMinimaxAgent(2, pokedata)
...
if __name__ == "__main__":
    ...
    #sd.test()
    '''
    win1cnt = len([name for name in os.listdir(DIR1+"/wins") if os.path.isfile(os.path.join(DIR1+"/wins", name)) and os.path.splitext(name)[1] == ".log"])
    los1cnt = len([name for name in os.listdir(DIR1+"/losses") if os.path.isfile(os.path.join(DIR1+"/losses", name)) and os.path.splitext(name)[1] == ".log"])
    while (win1cnt + los1cnt) < 20:
    '''
    main()
    '''
    win2cnt = len([name for name in os.listdir(DIR2+"/wins") if os.path.isfile(os.path.join(DIR2+"/wins", name)) and os.path.splitext(name)[1] == ".log"])
    los2cnt = len([name for name in os.listdir(DIR2+"/losses") if os.path.isfile(os.path.join(DIR2+"/losses", name)) and os.path.splitext(name)[1] == ".log"])
    print(los2cnt+win2cnt)
    #main()
    '''
